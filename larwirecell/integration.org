...
Get a build FW "control file" which lists packages and their versions to produce.  This list must match a targeted version of =larsoft=.  The starting point for this branch is [[./wire-0.5.2c]].  It must be consistent with the fresh-shell output of:
...
- The control file is linked here: [[./wire-0.5.2c]].  
